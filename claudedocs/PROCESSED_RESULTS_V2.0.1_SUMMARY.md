...
**版本**: v2.0.1
**完成时间**: 2025-11-03
**任务目标**: 扩展 `processed_results` 表支持 AI 服务新增字段，避免前端查询时 JOIN search_results 表

## 背景

### v2.0.0 架构问题
- **职责分离**: `search_results` (原始数据) + `processed_results` (AI 增强数据)
- **查询痛点**: 前端需要同时查询两个表并 JOIN，性能差
- **数据冗余**: AI 服务已经在 `processed_results` 添加了完整字段，但代码层未支持

### v2.0.1 解决方案
- **字段嵌入**: 在创建 `processed_results` 时复制 `search_results` 的所有原始字段
- **一站式查询**: 前端只需查询 `processed_results` 即可获得完整数据（原始+AI增强）
...
### 1. 性能提升
- **消除 JOIN**: 前端只需查询 `processed_results` 一张表
...
### 现有数据迁移
对于已有的 `processed_results` 记录（220条），可以选择：
...
创建迁移脚本补全原始字段：
```python
# scripts/migrate_processed_results_v201.py
async def backfill_original_fields():
    """为现有 processed_results 补全原始字段"""
    db = await get_mongodb_database()

    # 查询缺少原始字段的记录
    processed_results = db['processed_results'].find({
        "title": {"$exists": False}
    })

    async for record in processed_results:
        ...
        if raw_data:
            # 更新记录，添加原始字段
            await db['processed_results'].update_one(
                {"_id": record["_id"]},
                {"$set": {
...
### 已完成 ✅
1. ✅ 分析 processed_results 表现有字段结构
2. ✅ 修改 ProcessedResult 实体（添加原始字段+AI字段）
3. ✅ 修改 ProcessedResultRepository 的转换方法
4. ✅ 修改 TaskScheduler 在创建 processed_results 时复制原始字段
5. ✅ 创建测试脚本验证功能正常
...
### 数据库查询
```javascript
// 查看现有 processed_results 字段
db.processed_results.findOne({}, {_id: 0})

// 统计记录数
db.processed_results.countDocuments()

// 查看有原始字段的记录数
db.processed_results.countDocuments({"title": {"$exists": true}})
```
