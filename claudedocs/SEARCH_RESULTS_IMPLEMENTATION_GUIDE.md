...
**脚本**: `scripts/create_processed_results_indexes.py`

```python
"""创建 processed_results 集合索引"""
...
async def create_indexes():
    ...
    print("✅ processed_results 索引创建成功")
    client.close()
...
**执行**:
```bash
python scripts/create_processed_results_indexes.py
```
...
1. **添加新的查询端点（默认返回processed_results）**:
...
async def get_task_results(
    ...
    v2.0.0 变更：
    - 默认返回 processed_results（AI处理后的结果）
    - 支持 view=raw 查看原始结果
...
```python
"""
将现有 search_results 迁移到 processed_results
...
async def migrate():
    ...
    processed_results_col = db["processed_results"]
    ...
    while skip < total:
        ...
        async for doc in cursor:
            ...
            # 检查是否已存在
            existing = await processed_results_col.find_one({"raw_result_id": raw_id})
            if existing:
                skipped += 1
                continue
        ...
        # 批量插入
        if batch:
            await processed_results_col.insert_many(batch)
...
async def verify():
    ...
    processed_results_count = await db.processed_results.count_documents({})

    print(f"search_results 记录数: {search_results_count}")
    print(f"processed_results 记录数: {processed_results_count}")

    if search_results_count == processed_results_count:
        print("✅ 迁移验证成功：记录数一致")
    else:
        print("❌ 迁移验证失败：记录数不一致")
        print(f"   差异: {abs(search_results_count - processed_results_count)} 条")

    # 检查关联关系
    cursor = db.processed_results.aggregate([
        {
            "$lookup": {
                "from": "search_results",
