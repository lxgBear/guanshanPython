...
| AI处理结果 | 无 | `instant_processed_results` |
...
- 创建 `instant_processed_results` 表
...
- 新增 `instant_processed_results` 相关API
...
- [ ] 所有现有 `instant_search_results` 记录都有 `search_type="instant"`
- [ ] 所有 `smart_search_results` 数据已迁移到 `instant_search_results` (search_type="smart")
- [ ] 迁移后的数据字段完整，无缺失
- [ ] 聚合评分字段（composite_score, sources等）正确保留

### 功能验证

- [ ] 即时搜索功能正常，结果正确标记为 `search_type="instant"`
- [ ] 智能搜索功能正常，聚合结果正确标记为 `search_type="smart"`
- [ ] API查询支持 `search_type` 参数
- [ ] 前端两种查看模式（combined/by_query）正常工作
...
- [ ] `SmartSearchService` 已更新使用统一Repository
- [ ] `InstantSearchResultRepository` 支持 `search_type` 参数
- [ ] `InstantProcessedResultRepository` 创建完成
- [ ] 废弃的 `AggregatedSearchResultRepository` 标记 DEPRECATED
