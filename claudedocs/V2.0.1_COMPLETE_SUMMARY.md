...
### 5. ✅ 创建数据库索引（为 processed_results 添加索引）
- 创建 4 个性能优化索引:
...
5. **`scripts/create_processed_results_indexes.py`**
   - 创建 4 个索引
   - 性能验证
   - 支持幂等性
...
bulk_create_pending_results()
    ...
    └─ 创建 processed_results (原始+AI占位符)
    ↓
AI 服务处理
    └─ 填充 AI 字段（12个）
    ↓
前端查询 processed_results
    ├─ 使用索引加速查询 ⬅️ v2.0.1
...
ce6b3f5 feat: 为 processed_results 创建性能优化索引
b252932 docs: 添加 v2.0.1 字段扩展完整总结文档
7b0f782 test: 添加 v2.0.1 原始字段复制功能测试
c5c8a13 feat: v2.0.1 扩展 processed_results 支持原始字段嵌入
```
