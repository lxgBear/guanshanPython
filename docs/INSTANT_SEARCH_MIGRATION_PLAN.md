...
- 等待 AI 服务确定 `news_results` 字段需求
- 修改 `ProcessedResult` 实体添加原始字段
- 更新 `TaskScheduler` 复制原始字段到 `news_results`
- 修改 API 响应模型返回完整数据
- 添加用户操作 API（留存、删除、评分）
- AI 服务集成接口和异步处理
