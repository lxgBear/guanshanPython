...
**日期**: 2025-11-03
**版本**: v2.1.0（更新：添加news_results职责分离）
...
| `news_results` | **AI处理系统** | 存储定时搜索AI处理后的增强结果 | v2.0.0+ | 🚧 设计中 |
| `instant_search_results` | **即时+智能搜索** | 统一存储即时和智能搜索结果 | v1.3.0+ | ✅ 使用中 |
| `instant_news_results` | **AI处理系统** | 存储即时+智能搜索AI处理后的增强结果 | v2.1.0+ | 🚧 设计中 |
...
- **职责分离一致性**：两个系统都采用"原始数据 + AI处理结果"的双表架构
  - 定时搜索：`search_results` → `news_results`
  - 即时+智能搜索：`instant_search_results` → `instant_news_results`
...
- ✅ **统一AI处理表**：`instant_news_results` 统一处理两种搜索类型的AI增强
...
│   - instant_news_results (统一AI处理结果表) 🚧 设计中     │
...
- **统一AI处理表**：`instant_news_results`（v2.1.0 设计中）
...
```json
{
  ...
  // ❌ v2.0.0 移除: status, processed_at（迁移到news_results）
}
```
...
### 2. news_results（AI处理结果表，v2.0.0 新增）
...
### 4. instant_news_results（即时+智能搜索AI处理结果表，v2.1.0 新增）

**集合名称**：`instant_news_results`
...
4. ✅ **统一优势**：迁移到 `instant_search_results` 后，统一使用 `instant_news_results` 处理
...
|-----|---------|------------|--------------|------|
| **定时任务** | 定时搜索系统 | `search_results` | `news_results` | 每天早上8点搜索"AI新闻" |
| **立即搜索** | 即时搜索系统 | `instant_search_results`<br/>(`search_type="instant"`) | `instant_news_results` | 用户点击"立即搜索"按钮 |
| **智能搜索** | 智能搜索系统 | `instant_search_results`<br/>(`search_type="instant"` 子查询<br/>`search_type="smart"` 聚合) | `instant_news_results` | 用户输入"AI最新进展"，LLM分解为3个子查询 |
...
AI服务异步处理
    ↓
存储到 news_results（AI增强数据）
    ↓
前端查询 news_results（默认视图）
```
...
AI服务异步处理
    ↓
存储到 instant_news_results（AI增强数据）
    ↓
前端查询 instant_news_results（默认视图）
```
...
存储到 instant_news_results（AI增强数据）
    ↓
前端查询：
  - combined 模式：从 instant_news_results 读取（search_type="smart"）
  - by_query 模式：从 instant_news_results 读取（search_type="instant"）
```
...
| `ProcessedResultRepository` | `news_results` | `src/infrastructure/database/processed_result_repositories.py` | v2.0.0 | 🚧 设计中 |
...
| `InstantProcessedResultRepository` | `instant_news_results` | `src/infrastructure/database/instant_processed_result_repositories.py` | v2.1.0 | 🚧 设计中 |
...
- ✅ **AI处理统一**：新增 `instant_news_results` 表统一处理两种搜索类型
- ⚠️ **废弃计划**：`smart_search_results` 标记为待废弃，计划迁移数据
- ✅ **架构一致性**：定时搜索和即时+智能搜索都采用"原始 + AI处理"双表架构

**v2.0.0（2025-11-03）**：
- ✅ 定时搜索职责分离：`search_results` + `news_results`
- ✅ 原始数据不可变性：search_results只写一次
- ✅ AI异步处理：news_results管理AI处理状态和结果
