...
**脚本**: `scripts/create_news_results_indexes.py`

```python
"""创建 news_results 集合索引"""
...
**执行**:
```bash
python scripts/create_news_results_indexes.py
```
...
```python
"""
将现有 search_results 迁移到 news_results
...
async def migrate():
    ...
    news_results_col = db["news_results"]
    ...
    while skip < total:
        ...
        async for doc in cursor:
            ...
            # 检查是否已存在
            existing = await news_results_col.find_one({"raw_result_id": raw_id})
            if existing:
                skipped += 1
                continue
        ...
        # 批量插入
        if batch:
            await news_results_col.insert_many(batch)
...
async def verify():
    ...
    news_results_count = await db.news_results.count_documents({})

    print(f"search_results 记录数: {search_results_count}")
    print(f"news_results 记录数: {news_results_count}")

    if search_results_count == news_results_count:
        print("✅ 迁移验证成功：记录数一致")
    else:
        print("❌ 迁移验证失败：记录数不一致")
        print(f"   差异: {abs(search_results_count - news_results_count)} 条")

    # 检查关联关系
    cursor = db.news_results.aggregate([
        {
            "$lookup": {
                "from": "search_results",
