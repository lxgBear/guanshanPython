...
class MongoInstantProcessedResultRepository(IInstantProcessedResultRepository):
    ...
    async def create(self, entity: InstantProcessedResult) -> str:
        """创建即时搜索AI处理结果
        ...
        try:
            ...
            await (await self._get_collection()).insert_one(doc)
            logger.info(f"✅ 创建即时搜索AI处理结果: {entity.id}")
            return entity.id

        except Exception as e:
            logger.error(f"❌ 创建即时搜索AI处理结果失败: {entity.id}, 错误: {e}")
            raise RepositoryException(f"创建即时搜索AI处理结果失败: {e}")

    async def update(self, entity: InstantProcessedResult) -> bool:
        """更新即时搜索AI处理结果

        Args:
            entity: 要更新的即时搜索AI处理结果实体（必须包含有效的 ID）
        ...
        try:
            ...
            logger.error(f"❌ 更新即时搜索AI处理结果失败: {entity.id}, 错误: {e}")
            raise RepositoryException(f"更新即时搜索AI处理结果失败: {e}")
    ...
    async def delete(self, id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"删除即时搜索AI处理结果: {id}, 删除数量={result.deleted_count}")
            return result.deleted_count > 0

        except Exception as e:
            logger.error(f"❌ 删除即时搜索AI处理结果失败: {id}, 错误: {e}")
            raise RepositoryException(f"删除即时搜索AI处理结果失败: {e}")
    ...
    async def exists(self, id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"❌ 检查即时搜索AI处理结果是否存在失败: {id}, 错误: {e}")
            raise RepositoryException(f"检查即时搜索AI处理结果是否存在失败: {e}")
