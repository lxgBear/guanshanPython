...
processed_results_new 表 = AI处理结果（分析、翻译、增强）
```
...
| **processed_results_new** | AI处理后的结果 | AI服务处理 | AI服务写入、前端读取 |
...
**移除字段**:
- ❌ `status` - 业务状态管理移至processed_results_new
- ❌ `processed_at` - AI处理时间移至processed_results_new
...
### 2. ProcessedResult (processed_results_new 表)
...
│   2. 分析、翻译、总结                     │
│   3. 写入 processed_results_new              │
└────────────┬────────────────────────────┘
             │
             ↓ 保存AI结果
┌─────────────────────────────────────────┐
│   ProcessedResult (processed_results_new)   │
│   - AI翻译内容                           │
│   - AI总结摘要                           │
│   - 情感分析、分类标签                   │
│   - 用户操作状态                         │
...
1. **单向数据流**:
   ...
   - `processed_results_new` → AI服务异步写入

2. **职责清晰**:
   - 定时任务：负责获取原始数据并通知AI服务
   - AI服务：负责处理和增强数据
   - 前端：从 `processed_results_new` 读取最终数据

3. **状态管理**:
   - `search_results`: 无状态，纯数据存储
   - `processed_results_new`: 有状态（PENDING → PROCESSING → COMPLETED）
...
| **processed_results_new** | ✅ 前端/API | ✅ AI服务 | ✅ AI服务/用户 | ⚠️ 软删除 |
...
3. **关联关系**:
   - `processed_results_new.raw_result_id` 必须存在于 `search_results._id`
   - 级联删除：删除 `search_results` → 同时删除对应的 `processed_results_new`
...
- [ ] `/api/v1/search-tasks/{id}/results` 端点
  - 默认返回 `processed_results_new`（用户视角）
...
async def migrate():
    """
    将现有 search_results 中的状态数据迁移到 processed_results_new
...
1. 从 `search_results` 读取原始数据
2. 执行分析、翻译、总结
3. 更新 `processed_results_new` 状态为 PROCESSING
4. 保存AI结果到 `processed_results_new`
5. 更新状态为 COMPLETED
...
前端需要：
1. 修改查询接口从 `processed_results_new` 读取数据
...
**processed_results_new 索引**:
```javascript
db.processed_results_new.createIndex({ "raw_result_id": 1 }, { unique: true })
db.processed_results_new.createIndex({ "task_id": 1, "status": 1, "updated_at": -1 })
db.processed_results_new.createIndex({ "status": 1, "retry_count": 1 })  // 重试查询
```
...
1. **职责清晰**: search_results 纯数据存储，processed_results_new 业务逻辑
...
AI服务 → search_results (读) + processed_results_new (写)
前端 → processed_results_new (读写)
```
